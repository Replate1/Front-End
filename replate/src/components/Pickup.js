...
export const Pickup = props => {
  ...
  // console.log("Pickup card props: ", props);
  ...
  const { id, food_type, amount, pickup_time, completed, business_id } = props.pickup;
  console.log("Pickups.js props.pickup: ", props.pickup)
  ...
  return editing ?  (
    ...
    <button onClick={handleUpdate} >Submit</button>
      ...
      {userNum === 1 ? <button onClick={editPickupHandler}>Edit Pickup</button> : <button>Complete Pickup</button>}
      {userNum === 1 ? <button onClick={deletePickupHandler}>Delete Pickup</button> : <button>Remove Pickup</button>}
    </div>
  );
};
